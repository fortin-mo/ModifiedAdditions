...
public class TreeFeller
{
    ...
    public static void getBlocksToChop(Block relative, final Block block, final List<Block> list) {
        while (relative.getY() <= block.getY()) {
            ...
            if ((list.contains(relative.getRelative(BlockFace.UP))
                    || relative.getRelative(BlockFace.UP).getType() != Material.LOG)
                    && relative.getRelative(BlockFace.UP).getType() != Material.LOG_2)
                break;
    ...
    public static Block getHighestLog(Block block) {
        ...
        while (b) {
            if (block.getRelative(BlockFace.UP).getType().equals(Material.LOG)
                ...
                else if (block.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH_EAST).getType().equals(Material.LOG)
                        || block.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH_EAST).getType().equals(Material.LOG_2))
                    block = block.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH_EAST);

                else if (block.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH_WEST).getType().equals(Material.LOG)
                        || block.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH_WEST).getType().equals(Material.LOG_2))
                    block = block.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH_WEST);

                else if (block.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH_EAST).getType().equals(Material.LOG)
                        || block.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH_EAST).getType().equals(Material.LOG_2))
                    block = block.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH_EAST);

                else {
                    if (!block.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH_WEST).getType().equals(Material.LOG)
                            && !block.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH_WEST).getType().equals(Material.LOG_2))
                        continue;
    ...
    public static boolean isTree(Block relative, final Player player, final Block block) {
        ...
        if (relative.getRelative(BlockFace.UP).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.UP).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.UP).getRelative(BlockFace.UP).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.UP).getRelative(BlockFace.UP).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.UP).getRelative(BlockFace.NORTH).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.UP).getRelative(BlockFace.SOUTH).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.UP).getRelative(BlockFace.EAST).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.UP).getRelative(BlockFace.EAST).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.UP).getRelative(BlockFace.WEST).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.UP).getRelative(BlockFace.WEST).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.DOWN).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.DOWN).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.NORTH).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.NORTH).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.EAST).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.EAST).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.SOUTH).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.SOUTH).getType() == Material.LEAVES_2)
            ++n;

        if (relative.getRelative(BlockFace.WEST).getType() == Material.LEAVES
                || relative.getRelative(BlockFace.WEST).getType() == Material.LEAVES_2)
            ++n;
        ...
        while (n < list.size()) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable(){
                ...
                public void run() {
                    ...
                    int durability = 64;
                    CustomItem c = customItemStack.getCustomItem();

                    if (c.hasFakeDurability())
                        durability = customItemStack.getFakeDurability();
                    else
                        durability = customItemStack.getItemStack().getDurability();
        ...
        for (int i = -3; i < 4; ++i) {
            for (int j = -3; j < 4; ++j) {
                for (int k = -3; k < 4; ++k) {
                    ...
                    if (relative.getType().equals(Material.LEAVES) || relative.getType().equals(Material.LEAVES_2)) {
                        ++n;
                        Bukkit.getScheduler().scheduleSyncDelayedTask(TreeFeller.plugin, new Runnable() {
                            @Override
                            public void run() {
